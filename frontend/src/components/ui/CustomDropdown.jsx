    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`w-full flex items-center justify-between px-6 py-4 bg-neutral-50 border border-neutral-100 rounded-2xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-brand-500/20 ${isOpen ? 'border-brand-500 ring-2 ring-brand-500/10 shadow-sm' : 'hover:border-neutral-200'}`}
            >
                <div className="flex items-center gap-3">
                    {Icon && <Icon className={`transition-colors duration-300 ${isOpen ? 'text-brand-500' : 'text-neutral-400'}`} size={18} />}
                    <span className={`text-[13px] font-bold tracking-wide ${value && value !== 'all' ? 'text-slate-900' : 'text-neutral-400'}`}>
                        ...
                        <div className="max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-200 scrollbar-track-transparent">
                            {options.map((option) => (
                                <button
                                    ...
                                    className={`w-full text-left px-6 py-3.5 text-[13px] font-bold tracking-wide transition-all duration-200 flex items-center justify-between
