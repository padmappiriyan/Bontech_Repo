            ...
            <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left ${
                    isSelected
                        ...
                        : 'bg-white border border-transparent hover:bg-neutral-50 hover:border-neutral-100'
                }`}
            >
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-[12px] font-black flex-shrink-0 ${isSelected ? 'bg-brand-100 text-brand-600' : colorClass}`}>
                    {initials}
                </div>
                <div className="flex flex-col min-w-0">
                    <span className={`text-[13px] font-bold truncate ${isSelected ? 'text-brand-700' : 'text-neutral-700'}`}>
                        {user.name || 'No Name'}
                    </span>
                    <span className={`text-[11px] font-medium truncate ${isSelected ? 'text-brand-500' : 'text-neutral-400'}`}>
                        {user.email}
                    </span>
                </div>
                {isSelected && (
