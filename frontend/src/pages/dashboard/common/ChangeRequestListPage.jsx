...
const ChangeRequestListPage = () => {
    ...
    const { requests, analytics, loading, analyticsLoading } = useSelector(
        (state) => state.changeRequests
    );
    ...
    useEffect(() => {
        dispatch(fetchChangeRequests({ status: statusFilter }));
    }, [dispatch, statusFilter]);

    useEffect(() => {
        dispatch(fetchChangeRequestAnalytics());
    }, [dispatch]);
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {/* Filter */}
                        <button className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-[12px] font-semibold text-slate-600 hover:border-slate-300 transition-all shadow-sm">
                            <FiFilter size={13} className="text-slate-400" />
                            Filter
                        </button>

                        {/* New Change Request CTA */}
                        <button
                            onClick={() => navigate('/dashboard/change-requests/new')}
                            className="flex items-center gap-2 px-4 py-2 bg-brand-600 text-white rounded-xl text-[12px] font-bold shadow-md shadow-brand-500/20 hover:bg-brand-700 transition-all"
                        >
                            + New Request
                        </button>
                    </div>
